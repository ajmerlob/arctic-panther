...
ATTENDANCE_URI = 'attendance'
MEMBER_PHOTO_URI = '2/member_photo'
MESSAGES_URI = '2/message'

API_BASE_URL = 'http://api.meetup.com/'
SECURE_API_BASE_URL = 'https://secure.meetup.com/muapi/self/'
...
class Meetup(object):
    ...
    def get_attendance_list(self, urlname, id, **args):
        return API_Response(self._fetch("%s/events/%s/attendance" % (urlname,id), **args), ATTENDANCE_URI)

    def post_member_photo(self, **args):
        return self._post_multipart(MEMBER_PHOTO_URI, **args)

    def post_message(self, **args):
        return self._post(MESSAGES_URI, **args)
...
READ_METHODS = ['2/groups', '2/events', 'topics', '2/cities', '2/members', '2/rsvps',
                '2/photos', 'comments', 'activity']
...
for method in READ_METHODS:
    ...
    setattr(Meetup, 'get_' + method.strip("2/"), read_method)
...
class Event(API_Item):
    datafields =["announced", "announced_at", "comment_count", "created", "description", \
                 "distance", "duration", "email_reminders", "event_hosts", "event_url", \
                 "featured", "fee", "group", "headcount", "how_to_find_us", \
                 "id", "is_simplehtml", "maybe_rsvp_count", "name", "photo_album_id", \
                 "photo_count", "photo_url", "publish_status", "rating", "rsvp_alerts", \
                 "rsvp_limit", "rsvp_rules", "rsvpable", "self", "short_link", \
                 "simple_html_description", "status", "survey_questions", "time", "timezone", \
                 "trending_rank", "updated", "utc_offset", "venue", "venue_visibility", \
                 "visibility", "why", "yes_rsvp_count"]
...
#    datafields = ['name', 'link', 'comment','zip','coord','lon','city','state','country','response','guests','answers','updated','created']
    datafields = ['answers' , 'comments', 'created', 'event', 'group', 'guests', 'host', 'member', \
                  'member_photo','mtime','pay_status','response','rsvp_id','venue','watching']

    def __str__(self):
        try:
            return 'Rsvp by %s (%s) with comment: %s' % (self.member, self.member_photo,self.comments)
        except:
            try:
                return 'Rsvp by %s (%s)' % (self.member, self.member_photo)
            except:
                try:
                    return 'Rsvp by %s' % (self.member)
                except:
                    return 'Rsvp Anon'
            

class AttendanceList(API_Item):
    datafields = ['member', 'rsvp', 'status']

    def __str__(self):
        return 'Attendance List...'

class Group(API_Item):
    datafields = [ 'id','name','urlname','link','updated',\
